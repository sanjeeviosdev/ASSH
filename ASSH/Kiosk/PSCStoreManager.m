...
- (void)downloadMagazine:(PSCMagazine *)magazine {
   ...
   // sanjeev stop downloading method
...
- (NSMutableArray *)searchFolder:(NSString *)sampleFolder {
   ...
   // NSArray *documentContents=[[NSArray alloc] init];
    NSArray *documentContents = [fileManager contentsOfDirectoryAtPath:sampleFolder error:&error];
    
    
    if (UIAPPDelegate.isSorting==YES) {
        
        documentContents = [documentContents sortedArrayUsingSelector:@selector(localizedCompare:)];
    }
...
- (NSMutableArray *)searchForMagazineFolders {
    ...
    if ([UIAPPDelegate isMyTopic] == YES) {
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentsDirectory = [paths objectAtIndex:0];
        searchFolder = [documentsDirectory stringByAppendingPathComponent:@"MyTopics"];
    }
    else {
        NSArray *paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
        NSString *documentsDirectory = [paths objectAtIndex:0];
        searchFolder = [documentsDirectory stringByAppendingPathComponent:@"Samples"];

        
        //searchFolder = [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"Samples"];
    ...
    */
    [folders addObjectsFromArray:[self searchFolder:searchFolder]];
